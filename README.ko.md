...
워크리스트에서 승인까지의 작업자 흐름 — 게이팅 결정과 모든 상태를 덮어쓰는 안전 분기 포함:

```mermaid
flowchart TD
  A(["워크리스트에서 검사 열기"]) --> B{"기기 연결됨?"}
  B -- 아니오 --> B1["설정 → 카메라 연결<br/>(벤더 · 시리얼)"] --> B
  B -- 예 --> C{"보정됨?"}
  C -- 아니오 --> C1["빈 침대 보정<br/>(GTS-002)"] --> C
  C -- 예 --> D["세션 시작"]
  D --> E["호흡 추적<br/>dZ/dt · d²Z/dt²"]
  E --> F["호흡 정지 큐"]
  F --> G{"안정 ≥1000ms?<br/>|dZ/dt|<2 · var<0.03 · px>0.85"}
  G -- "기침/급변 (d²Z/dt²>25)" --> H["중단(Abort)"] --> E
  G -- "타임아웃 10s" --> T["시간 초과"] --> E
  G -- 예 --> I["ready_to_capture"]
  I --> J{"LUT 행 임상 검토 대기?"}
  J -- 예 --> K["수동 검토 필요<br/>(kVp/mAs 없음)"] --> E
  J -- 아니오 --> L["kVp/mAs → 가드레일 클램프<br/>kVp 60–120 · mAs 1–80"]
  L --> M{"작업자 승인?"}
  M -- 아니오 --> E
  M -- 예 --> N["감사 기록 → X선 장비 고유의<br/>수동 워크플로에서 작업자가 적용"]
  E -. "결함 발생" .-> S[["세이프 스테이트 / 수동 모드<br/>권고 비활성화"]]
  S -. "정상 프레임" .-> E
```

시스템은 **감사 기록에서 멈춥니다** — X선을 발사하지 않습니다. [깊이 & 게이팅](docs/depth-and-gating.ko.md) · [노출 & 안전](docs/exposure-and-safety.ko.md) 참고.
